##########################################
## FUNCTION TO BUILD DEEP MODEL
##########################################

build_deep_h2o <- function(model_name,train_data,test_data,output_dir,y_var='target',x_vars=names(train_data)) {
  ...
  while( is.null(mod) && attempt <= 3 ) {
    attempt <- attempt + 1
    tryCatch(
      mod <- h2o.deeplearning(y=y_var, x=x_vars, training_frame=train_data, variable_importances=T),
